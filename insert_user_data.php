<?php
  ...
  //mysqli_query($db_conn, $sql); // 데이터 베이스에 작업을 요청 : 첫번째 파라미터는 접속정보, 두번째 파라미터는 요청작업

  // 2. 데이터 조회 
  // SELECT [field name](* : 전체조회) FROM [tablename];
  ...
  // 2-1. 데이터 필터링 조회 
  // SELECT [field name](* : 전체조회) FROM [tablename] WHERE [fieldname] = [fieldvalue];
  //$sql_select = "SELECT * FROM user WHERE user_name = '토토'";
  // $sql_select = "SELECT * FROM user WHERE user_idx = 2";
  // $result = mysqli_query($db_conn, $sql_select);
  // 2-2. 데이터 정렬
  // SELECT [field name](* : 전체조회) FROM [tablename] ORDER BY [fieldname] DESC(역순), ASC(정순) 
  // $sql_select = "SELECT * FROM user ORDER BY user_idx DESC";
  // $result = mysqli_query($db_conn, $sql_select);
  // 2-3. 데이터 갯수 지정 조회
  // SELECT [field name](* : 전체조회) FROM [tablename] LIMIT [start index], [number(length)]
  // $sql_select = "SELECT * FROM user WHERE user_name = '토토' ORDER BY user_idx DESC LIMIT 2, 5";
  // $result = mysqli_query($db_conn, $sql_select);
  ...
  // - 중복된 이름 하나 선택해서 삭제
  $sql_del = "DELETE FROM user WHERE user_name = '차혜지'";
